...
export async function POST(req: NextRequest) {
  try {
    ...
    const color = TIER_COLOR[tier] || '#facc15';
    ...
    const { error } = await resend.emails.send({
      ...
      html: `
        <div style="background:#08080d;color:#fff;font-family:monospace;max-width:480px;margin:0 auto;padding:40px 32px;border-radius:16px;">
          <h1 style="font-size:2rem;font-weight:900;margin:0 0 8px;color:#facc15;">Do You Know Ball?</h1>
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://youknoball.com'}"
             style="display:inline-block;background:#facc15;color:#000;font-weight:900;padding:14px 28px;border-radius:10px;text-decoration:none;font-size:0.95rem;">
            Come Back Tonight →
          </a>
