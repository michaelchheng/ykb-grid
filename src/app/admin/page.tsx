      ...
      <div className="min-h-screen bg-[#08080d] text-white px-4 py-10">
        <div className="w-full max-w-2xl mx-auto space-y-6">
          ...
          <div className="text-center">
            <p className="text-emerald-400/60 text-[10px] font-mono uppercase tracking-widest mb-2">Admin Panel</p>
            <h1 className="text-2xl font-black">You Know Ball <span className="text-emerald-400">Admin</span></h1>
          </div>

          {/* Status bar */}
          <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 flex items-center justify-between">
            <div>
              <p className="text-emerald-400 text-xs font-mono font-bold uppercase tracking-wider">Admin Mode Active</p>
              <p className="text-white/40 text-xs mt-0.5">Lockouts bypassed on all modes</p>
            </div>
            <Link href="/" className="text-emerald-400 text-xs font-bold border border-emerald-500/30 rounded-lg px-3 py-1.5 hover:bg-emerald-500/10 transition-colors">
              Play →
            </Link>
          </div>
          ...
          <div className="flex gap-1 rounded-xl border border-white/8 bg-white/[0.03] p-1">
            {(['tools', 'pipeline', 'feedback'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={['flex-1 py-2 rounded-lg text-xs font-bold capitalize transition-colors', activeTab === tab ? 'bg-emerald-500 text-black' : 'text-white/40 hover:text-white/70'].join(' ')}>
                      ...
                      <div key={i} className="rounded-xl border border-white/6 bg-black/20 p-3 space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            <span className="text-emerald-400 text-[10px] font-bold uppercase">{log.difficulty}</span>
                    ...
                    {stats.questionStats.map((q, i) => (
                      <div key={i} className="rounded-xl border border-white/6 bg-black/20 px-3 py-2.5 flex items-center justify-between gap-2">
                        ...
                        <div className="flex gap-3 text-[10px] font-mono shrink-0">
                          <span className={q.accuracy >= 60 ? 'text-green-400' : q.accuracy >= 35 ? 'text-emerald-400' : 'text-red-400'}>
                            {q.accuracy}%
                          </span>
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-3">
          <input
            ...
            className="w-full bg-white/[0.04] border border-white/12 rounded-xl px-4 py-3.5 text-white text-sm placeholder:text-white/25 outline-none focus:border-emerald-500/40 transition-colors"
          />
          <input
            type="password"
            value={password}
          ...
          <button type="submit"
            className="w-full py-3.5 rounded-xl bg-emerald-500 text-black text-sm font-black hover:bg-emerald-400 transition-colors active:scale-[0.99]">
            Sign In
          </button>
        </form>
