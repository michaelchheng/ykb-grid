      ...
      <div className="max-w-sm w-full">
          ...
          {(['create', 'join'] as const).map(m => (
            <button key={m} onClick={() => { setMode(m); setError(''); }}
              ...
              style={{
                ...
                border: `1px solid ${mode === m ? 'rgba(56,189,248,0.4)' : 'rgba(255,255,255,0.1)'}`,
                color: mode === m ? '#38bdf8' : 'rgba(255,255,255,0.4)',
              }}>
        ...
        {mode === 'create' ? (
          <button onClick={createRoom}
            className="w-full py-4 rounded-xl bg-sky-400 text-black font-black text-sm hover:bg-sky-300 transition-colors active:scale-[0.98]">
            Create Room
          </button>
        ) : (
          <div className="space-y-3">
            <input
              ...
              maxLength={4}
              className="w-full rounded-xl border border-white/15 bg-white/[0.04] px-4 py-3 text-center text-2xl font-black font-mono tracking-[0.5em] placeholder:text-white/20 placeholder:tracking-widest outline-none focus:border-sky-400/50"
            />
  ...
  if (phase === 'waiting') return (
    <div className="min-h-screen bg-[#08080d] text-white flex flex-col items-center justify-center px-5">
      <div className="max-w-sm w-full text-center">
        ...
        <div className="text-[5rem] font-black leading-none tracking-[0.15em] text-sky-300 mb-6">{roomCode}</div>
      ...
      <div className="max-w-sm w-full text-center">
        ...
        <div className="flex items-center justify-center gap-6 mb-10">
          {players.map((p, i) => (
            <div key={i} className="text-center">
              <div className="w-14 h-14 rounded-full border-2 border-sky-400/30 bg-sky-400/10 flex items-center justify-center mx-auto mb-2">
                <span className="text-xl font-black">{p.username[0]?.toUpperCase()}</span>
              </div>
              <p className="font-black text-sm">{p.username}</p>
              {p.username === username && <p className="text-[10px] text-sky-300 font-mono">YOU</p>}
            </div>
          ))}
          {players.length === 2 && (
        ...
        {isHost ? (
          <button onClick={sendNextQuestion}
            className="px-10 py-4 rounded-xl bg-sky-400 text-black font-black text-sm hover:bg-sky-300 transition-colors active:scale-[0.98]">
            Start Game →
          </button>
        ) : (
    ...
    <div className="min-h-screen bg-[#08080d] text-white flex flex-col">
      ...
      <div className="border-b border-white/[0.06] px-5 py-3">
        <div className="flex items-center justify-between max-w-sm mx-auto">
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-black" style={{ color: timer <= 5 ? '#f87171' : '#38bdf8' }}>{timer}s</p>
          </div>
          <div className="text-center">
        ...
        <div className="flex gap-1 mt-3 max-w-sm mx-auto">
          {Array.from({ length: totalRounds }).map((_, i) => (
            <div key={i} className="flex-1 h-1 rounded-full"
              style={{ background: i < round - 1 ? '#38bdf8' : i === round - 1 ? 'rgba(56,189,248,0.4)' : 'rgba(255,255,255,0.08)' }} />
          ))}
        </div>
      </div>
      ...
      {opponentAnswered && !answered && (
        <div className="text-center pt-3">
          <p className="text-[10px] font-mono text-sky-300/60 uppercase tracking-widest">
      ...
      <div className="min-h-screen bg-[#08080d] text-white flex flex-col items-center justify-center px-5">
        <div className="max-w-sm w-full text-center">
          ...
          {!roundResult.gameOver && isHost && (
            <button onClick={sendNextQuestion}
              className="px-10 py-3 rounded-xl bg-sky-400 text-black font-black text-sm hover:bg-sky-300 transition-colors">
              Next Round →
            </button>
          )}
          {!roundResult.gameOver && !isHost && (
        ...
        <div className="max-w-sm w-full text-center">
          {opponentLeft ? (
            ...
            <>
              ...
              <p className="text-3xl font-black mb-2" style={{ color: iWon ? '#34d399' : isDraw ? '#38bdf8' : '#f87171' }}>
